...
- The Contact token area should read as neutral elevation metadata: blank or off-white sample surfaces plus literal token text. Do not let the model turn Contact 0-3 into a color ramp, semantic chip set, avatar list, or status palette.
...
- Table regression check: if that operating surface shows explicit column headers or rigid repeated cells, it has slipped back into admin UI and should be rejected.
...
- Radius consistency: cards, rows, inputs, buttons, tabs, chips, and icon buttons should follow a coherent radius scale and match the design-system strip. In the default premium direction, rounded modern controls are the safer baseline; use zero/sharp corners only when they are clearly intentional, and avoid the middling admin radius zone, especially visible 4px/6px/8px-style control radii.
