...
6. Generate the first design board in light mode.
   ...
   - Also reject the board if buttons, segmented controls, tabs, or small interactive surfaces fall into a middling enterprise radius that makes the UI feel default or admin-like. The default premium direction should prefer distinctly rounded/modern controls with generous radius or pill treatment; visible 4px/6px/8px-style control radii usually read too timid and administrative unless the whole system is intentionally sharp. Use zero or near-zero radius only when the user's strict style or a clearly intentional sharp editorial system calls for it. Avoid the vague in-between look entirely.
...
- Do not accept a generated mock where buttons or small controls use a middling enterprise radius that makes the interface feel default/admin. The default premium style should lean clearly rounder and more modern; visible 4px/6px/8px-style control radii usually still feel administrative. Reserve zero or near-zero radius for explicit sharp-style direction, and apply the chosen radius language consistently.
